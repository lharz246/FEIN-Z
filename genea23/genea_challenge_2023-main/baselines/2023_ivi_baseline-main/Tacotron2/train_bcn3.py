...
from common.hparams_bcn3 import create_hparams as ch_bcn4
from common.logger import Tacotron2Logger
from torch.utils.data import DataLoader
from common.loss_bcn3 import BCGLoss
from common.utils import to_gpu
from common.bcn3 import BCNetwork, Discriminator
...
def validate(model, criterion, val_loader, iteration, dyadic, logger, warmup_period):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            x = (audio, text, output_lengths,
                 absolut_pos, gestures_in)
...
def train(output_directory, log_directory, checkpoint_path, warm_start, hparams):
    ...
    sequence_length = hparams.sequence_length - warmup_period
    dyadic = hparams.dyadic
    model_params = hparams.model_dyadic if dyadic else hparams.monadic_model
    torch.manual_seed(hparams.seed)
    torch.cuda.manual_seed(hparams.seed)

    model = load_model(model_params)
    discriminator = Discriminator(input_length=sequence_length, action_dim=action_dim).cuda()
    ...
    for i, batch in enumerate(train_loader):
        ...
        x = (audio[:, :, warmup_period:], text[:, :, warmup_period:], output_lengths - warmup_period,
             absolut_pos + warmup_period, gestures_in[:, :, :warmup_period])
        gestures_pred = model(x)
        gestures = gestures[:, :, warmup_period:]
        loss = criterion(gestures_pred, gestures)
        fake_validity = discriminator(gestures_pred.permute(0, 2, 1))
        real_validity = discriminator(gestures.permute(0, 2, 1))
        gradient_penalty = compute_gradient_penalty(discriminator, gestures.permute(0, 2, 1), gestures_pred.permute(0, 2, 1))
        ...
        if (iteration + 1) % hparams.iters_per_checkpoint == 0:
            print()
            duration = 0.
            validate(model, criterion, val_loader, iteration + 1, dyadic=dyadic, logger=logger,
