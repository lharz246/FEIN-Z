    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            if dyadic:
                ...
                gestures_in = torch.cat([gestures[:, :, :warmup_period], inter_gestures[:, :, :warmup_period]], dim=1)
                x = (audio, text, output_lengths - warmup_period, absolut_pos + warmup_period, gestures_in)
                gestures_pred = model.inference(x)
                gestures = torch.cat([gestures, inter_gestures], dim=1)
...
def train(output_directory, log_directory, checkpoint_path, warm_start, hparams):
    ...
    discriminator = Discriminator(input_length=200, action_dim=action_dim).cuda()
    discriminator.train()
