...
from common.bcn import BCNetwork
from common.hparams_bcn import create_hparams
...
model = BCNetwork(hparams.model_dyadic, dyadic=True)
model.eval()
...
for index in tqdm(range(1)): #len(h5.keys())
    ...
    with torch.no_grad():
        y_pred = model.validate_dyadic(input_x)
        predicted_gesture, _ = y_pred
        print(f'loss: {criterion(predicted_gesture.transpose(1, 2), motion)}')
