...
class SpeechGestureDataset_Dyadic(torch.utils.data.Dataset):
    def __init__(self, trn_h5file_main=None, trn_h5file_iloctr=None,
                 ...
                 sequence_length=200, npy_root="..", motion_dim=78):
    ...
    def __getitem__(self, idx):
        ...
        return text, audio, gesture, text_interlocutor, audio_interlocutor, gesture_interlocutor, length, start_frame
...
class SpeechGestureDataset_Dyadic_ValSequence(SpeechGestureDataset_Dyadic):
    def __getitem__(self, idx):
        ...
        return text, audio, gesture, text_interlocutor, audio_interlocutor, gesture_interlocutor, length, 0
...
def prepare_dataloaders(hparams):
    ...
    val_loader = DataLoader(val_dataset, num_workers=6,
...
def load_model(hparams, dyadic):
    model = BCNetwork(hparams, dyadic).cuda()
    return model
...
def parse_batch(batch):
    text, audio, gesture, text_interlocutor, audio_interlocutor, gesture_interlocutor, length, idx = batch
    ...
    return (audio, text, audio_interlocutor, text_interlocutor), (gesture, gesture_interlocutor), (output_lengths, idx)


def validate(model, criterion, val_loader, iteration, logger, dyadic, warmup_period):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            if dyadic:
                ...
                output_lengths, absolut_pos = additional
                gestures, inter_gestures = gestures
                audio = audio[:, :, warmup_period:]
                text = text[:, :, warmup_period:]
                audio_interlocutor = audio_interlocutor[:, :, warmup_period:]
                text_interlocutor = text_interlocutor[:, :, warmup_period:]
                gestures_in = gestures[:, :, :warmup_period]
                gestures_in_inter = inter_gestures[:, :, :warmup_period]
                x = (audio, text, audio_interlocutor, text_interlocutor, output_lengths - warmup_period,
                     absolut_pos + warmup_period, gestures_in, gestures_in_inter)
                gestures_pred = model.inference(x)
                gestures = torch.cat([gestures[:, :, warmup_period:], inter_gestures[:, :, warmup_period:]], dim=1)
                loss_bc = criterion(gestures_pred.transpose(1, 2), gestures)

            else:
                audio, text, _, _ = input_data
                gestures, _ = gestures
                output_lengths, absolut_pos = additional
                x = (audio[:, :, warmup_period:], text[:, :, warmup_period:], output_lengths - warmup_period,
                     absolut_pos + warmup_period,
                     gestures[:, :, :warmup_period])
                gestures_pred = model.inference(x)
                loss_bc = criterion(gestures_pred.transpose(1, 2), gestures)

            reduced_val_loss = loss_bc
...
def train(output_directory, log_directory, checkpoint_path, warm_start, hparams):
    ...
    action_dim = 336 if dyadic else 168
    model_params = hparams.model_dyadic if dyadic else hparams.model_monadic
    torch.manual_seed(hparams.seed)
    torch.cuda.manual_seed(hparams.seed)
    model = load_model(model_params, dyadic)
    model.train()
    learning_rate = hparams.learning_rate

    discriminator = Discriminator(input_length=100, action_dim=action_dim).cuda()
    discriminator.train()
    optimizer_disc = torch.optim.AdamW(discriminator.parameters(), lr=learning_rate, weight_decay=hparams.weight_decay)
    ...
    for i, batch in enumerate(train_loader):
        ...
        if dyadic:
            ...
            output_lengths, absolut_pos = additional
            gestures, inter_gestures = gestures
            x = (audio[:, :, warmup_period:], text[:, :, warmup_period:], audio_interlocutor[:, :, warmup_period:],
                 text_interlocutor[:, :, warmup_period:], output_lengths - warmup_period,
                 absolut_pos + warmup_period, gestures[:, :, :warmup_period], inter_gestures[:, :, :warmup_period])
            gestures_pred, gestures_pred_inter = model(x)
            gestures_pred = torch.cat([gestures_pred, gestures_pred_inter], dim=-1)
            gestures = torch.cat([gestures[:, :, warmup_period:], inter_gestures[:, :, warmup_period:]],
                                 dim=1)
        ...
        if not math.isnan(loss):
            ...
            print(
                f"Iteration: {iteration + 1} Loss: {loss:4f} d_Loss: {d_loss:4f}"
                f"  {duration / iters_from_last_save:2f}s/it")
            logger.log_training(reduced_loss=loss, d_loss=d_loss, learning_rate=learning_rate,
                                duration=duration / iters_from_last_save, iteration=iteration + 1)

        if (iteration + 1) % hparams.iters_per_checkpoint == 0:
            print()
            duration = 0.
            validate(model, criterion, val_loader, iteration + 1, logger, dyadic=dyadic, warmup_period=warmup_period)
...
if __name__ == '__main__':
    ...
    print(os.getcwd())
    os.chdir(
        '/media/compute/homes/lharz/genea23/genea_challenge_2023-main/baselines/2023_ivi_baseline-main/Tacotron2/')  # /media/compute/homes/lharz  '/media/compute/home/lharz/genea23/genea_challenge_2023-main/baselines/2023_ivi_baseline-main/Tacotron2/')
    torch.cuda.set_device("cuda:{}".format(hparams.device))
    torch.backends.cudnn.enabled = hparams.cudnn_enabled
    torch.backends.cudnn.benchmark = hparams.cudnn_benchmark

    train(hparams.output_directory, hparams.log_directory,
          hparams.checkpoint_path, hparams.warm_start,
          hparams)
